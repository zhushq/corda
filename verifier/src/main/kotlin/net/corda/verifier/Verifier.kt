...
class Verifier {
    companion object {
        ...
        fun loadConfiguration(baseDirectory: Path, configFile: Path): VerifierConfiguration {
            ...
            val customConfig = ConfigFactory.parseFile(configFile.toFile(), options)
            val finalConfig = ConfigFactory.parseMap(mapOf("baseDirectory" to baseDirectory.toString()))
                    .withFallback(customConfig)
                    .withFallback(defaultConfig)
                    .resolve()
            return finalConfig.parseAs<VerifierConfiguration>()
        }
