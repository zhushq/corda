 ...
 */
fun <T : Any> serializedHash(x: T, privacySalt: PrivacySalt?, index: Int, indexInternal: Int): SecureHash {
...
fun computeNonce(privacySalt: PrivacySalt, index: Int, indexInternal: Int) = (privacySalt.bytes + ByteBuffer.allocate(4).putInt(index).array() + ByteBuffer.allocate(4).putInt(indexInternal).array()).sha256()
