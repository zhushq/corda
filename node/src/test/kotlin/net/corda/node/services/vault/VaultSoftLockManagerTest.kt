...
class VaultSoftLockManagerTest {
    ...
    private val mockNet = MockNetwork(cordappPackages = listOf(ContractImpl::class.packageName), defaultFactory = { args ->
        object : MockNetwork.MockNode(args) {
            override fun makeVaultService(keyManagementService: KeyManagementService, stateLoader: StateLoader, attachments: AttachmentStorage, hibernateConfig: HibernateConfiguration): VaultServiceInternal {
                val realVault = super.makeVaultService(keyManagementService, stateLoader, attachments, hibernateConfig)
