...
class NetworkRegistrationHelper(private val config: NodeConfiguration, private val certService: NetworkRegistrationService) {
    ...
    init {
        ...
        val rootCert = config.openTrustStore().internal.getCertificate(CORDA_ROOT_CA)
    ...
    fun buildKeystore() {
        ...
        val caKeyStore = config.openNodeKeyStore(createNew = true)
        if (!caKeyStore.containsAlias(CORDA_CLIENT_CA)) {
            ...
            if (!caKeyStore.containsAlias(SELF_SIGNED_PRIVATE_KEY)) {
                ...
                caKeyStore.setPrivateKey(SELF_SIGNED_PRIVATE_KEY, keyPair.private, listOf(selfSignCert.cert), keyPassword = privateKeyPassword)
                caKeyStore.save()
            }

            val keyPair = caKeyStore.getCertificateAndKeyPair(SELF_SIGNED_PRIVATE_KEY, privateKeyPassword).keyPair
            ...
            caKeyStore.setPrivateKey(CORDA_CLIENT_CA, keyPair.private, certificates, keyPassword = privateKeyPassword)
            ...
            config.openSslKeyStore(createNew = true).update {
                val caCert = caKeyStore.getCertificate(CORDA_CLIENT_CA)
                val sslKey = Crypto.generateKeyPair(X509Utilities.DEFAULT_TLS_SIGNATURE_SCHEME)
                val sslCert = X509Utilities.createCertificate(
                        CertificateType.TLS,
                        caCert.toX509CertHolder(),
                        keyPair,
                        CordaX500Name.build(caCert.subjectX500Principal),
                        sslKey.public)
                setPrivateKey(CORDA_CLIENT_TLS, sslKey.private, listOf(sslCert.cert) + certificates)
            }
